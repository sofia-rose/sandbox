/*
  This code is based on pages 287 of the book:
...
void ring_persistent(
  ...
  MPI_Send_init(x, buff_count, MPI_DOUBLE, next, 3, MPI_COMM_WORLD, &send_request);

  MPI_Request recv_request;
  MPI_Recv_init(incoming, buff_count, MPI_DOUBLE, prev, 3, MPI_COMM_WORLD, &recv_request);
...
int main(int argc, char* argv[])
{
  ...
  ring_persistent(x, incoming, buff_count, proc_count, num_shifts, proc_id);
